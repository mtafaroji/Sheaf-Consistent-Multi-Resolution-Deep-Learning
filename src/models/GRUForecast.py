import torch
import torch.nn as nn


class GRUForecast(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=64, num_layers=2, T_out=16):
        super().__init__()

        self.T_out = T_out

        # GRU به جای LSTM
        self.gru = nn.GRU(
            input_dim,
            hidden_dim,
            num_layers=num_layers,
            batch_first=True
        )

        self.fc = nn.Linear(hidden_dim, T_out * 3)

    def forward(self, x, params):
        """
        x: (B, K, 3)  -> S, I, R
        params: (B, 3)
        """

        B, K, _ = x.shape

        # ------------------------
        # broadcast params
        # ------------------------
        params_expanded = params.unsqueeze(1).repeat(1, K, 1)  # (B,K,3)

        x_in = torch.cat([x, params_expanded], dim=-1)  # (B,K,6)

        # ------------------------
        # GRU
        # ------------------------
        out, _ = self.gru(x_in)  # (B,K,H)

        h_last = out[:, -1, :]  # (B,H)

        # ------------------------
        # fully connected
        # ------------------------
        y = self.fc(h_last)  # (B, T_out*3)

        y = y.view(B, self.T_out, 3)

        # ------------------------
        # residual (خیلی مهم مثل مدل خودم)
        # ------------------------
        last_state = x[:, -1, :].unsqueeze(1)  # (B,1,3)

        y = last_state + y

        return y