...
model_path = "outputs/models/best_TCN_Osc_16.pt"
...
# Load model
# --------------------------------------------------
model = TCNForecast(T_out=T_out).to(device)
...
plt.title(f"Predicted vs Original (windowed, run_id={run_id})")
plt.xlabel("Time")
plt.ylabel("State")
...
np.savez("outputs/data/TCN.npz", pred_TCN=pred_full[start:end, :])