...
from src.models.LSTMForecast import LSTMForecast
...
model_path = "outputs/models/best_lstm_Osc_248.pt"
...
# Load model
# --------------------------------------------------
model = LSTMForecast(T_out=T_out).to(device)
...
plt.title(f"LSTM Full Trajectory (Test run_id={run_id})")
plt.xlabel("Time")
plt.ylabel("State")
...
np.savez("outputs/data/LSTM.npz", pred_LSTM=pred_full[start:end, :], original=traj_norm[start:end, :])