...
states_df_noisy = pd.read_csv("data/raw/states.csv")
...
run_df_noisy = states_df_noisy[states_df_noisy["run_id"] == run_id].sort_values("t")
...
original_noisy = run_df_noisy[["S", "I", "R"]].values
original_noisy = original_noisy[K:]
original_noisy = (original_noisy - dataset.state_mean) / dataset.state_std
...
T_plot = min(240, len(pred_Aj))

original = original[8:T_plot]
original_noisy = original_noisy[8:T_plot]
pred_Aj  = pred_Aj[8:T_plot]
pred_Aj1 = pred_Aj1[8:T_plot]
pred_Aj2 = pred_Aj2[8:T_plot]
...
titles = [
    "Aj Prediction vs Original and Noisy Input",
    "Aj1 Prediction vs Original and Noisy Input",
    "Aj2 Prediction vs Original and Noisy Input"
]
...
for i in range(3):
    ...
    #plt.plot(t, original[:, 0], color="blue",label="True S1")
    #plt.plot(t, original_noisy[:, 0], "--", color="magenta", label="Noisy input S1")
    #plt.plot(t, preds[i][:, 0], "--", color="green",label="Pred S1")

    #plt.plot(t, original[:, 1], color="blue", label="True S2")
    #plt.plot(t, original_noisy[:, 1], "--", color="magenta", label="Noisy input S2")
    #plt.plot(t, preds[i][:, 1], "--", color="green", label="Pred S2")

    plt.plot(t, original[:, 2], color="blue", label="True S3")
    plt.plot(t, original_noisy[:, 2], "--", color="magenta", label="Noisy input S3")
    plt.plot(t, preds[i][:, 2], "--", color="green", label="Pred S3")
