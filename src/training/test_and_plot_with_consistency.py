...
for i in range(3):
    axs[i].grid()
    axs[i].plot(t, original[:, 0], label="True S1")
    axs[i].plot(t, preds[i][:, 0], "--", label="Pred S1")

    axs[i].plot(t, original[:, 1], label="True S2")
    axs[i].plot(t, preds[i][:, 1], "--", label="Pred S2")

    axs[i].plot(t, original[:, 2], label="True S3")
    axs[i].plot(t, preds[i][:, 2], "--", label="Pred S3")
...
np.savez("outputs/data/ML_Aj2.npz", pred_ML_Aj2=pred_Aj2)
